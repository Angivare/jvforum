...
function adaptMessageContent(content, id, authorNickname, postDateRaw) {
  ...
  while (matches = regex.exec(content)) {
    let url = matches.slice(1).join('').replace(/\$/g, '$$$$')
      , replace = `<a class="link" href="${url}" target="_blank" title="${url}">${url}</a>`
  ...
  regex = /<a class="link" href="((?:&#[x0-9a-f]+;)+)"[^<]+>([^<]+)<\/a>/g
  while (matches = regex.exec(content)) {
    let email = entities.decode(matches[1]).replace(/\$/g, '$$$$')
      , replace = `<a class="link" href="mailto:${email}" target="_blank" title="${email}">${email}</a>`
    content = content.replace(matches[0], replace)
    regex.lastIndex += -matches[0].length + replace.length
  }

  // NoelShack thumbnails
  content = content.replace(/<a(?: class="[^"]+")? href="(?:https?:)([^"]+)" data-def="NOELSHACK" target="_blank"><img class="img-shack" width="68" height="51" src="([^"]+)" alt="[^"]+"\/><\/a>/g, (all, href, src) => {
    // Custom stickers
  ...
  content = content.replace(/<a(?: class="[^"]+")? href="(https?:\/\/(?:www|m)\.jeuxvideo\.com\/forums\/([0-9]+)-([0-9]+)-([0-9]+)-([0-9]+)-0-1-0-([0-9a-z-]+)\.htm(?:\#post_([0-9]+))?)"[^>]+>([^<]+)<\/a>/g, (all, url, mode, forumId, topicIdlegacyOrModern, page, slug, messageId, text) => {
    let path = '/' + forumId
    ...
    return `<a class="link" href="${path}" title="${url}" data-link-jvc="${url}">jvforum.fr${path}</a>`
  })
  ...
  content = content.replace(/<a(?: class="[^"]+")? href="(https?:\/\/(?:[a-z]+\.)?youtube\.com\/watch[^"]*(?:\?|&amp;)v=([a-zA-Z0-9-_]{11})([^"])*)"[^>]+>[^<]+<\/a>/g, '<a class="youtube-link" href="$1" target="_blank" title="$1"><img class="youtube-link__thumb" src="https://img.youtube.com/vi/$2/mqdefault.jpg" alt="$1"></a>')
  ...
  content = content.replace(/<div class="player-contenu"><div class="embed-responsive embed-responsive-16by9"><iframe frameborder="0" width="[0-9]+" height="[0-9]+" src="https:\/\/www\.dailymotion\.com\/embed\/video\/([^"]+)" allowfullscreen><\/iframe><\/div><\/div>/g, '<a class="link" href="https://dai.ly/$1" target="_blank" title="https://dai.ly/$1">https://dai.ly/$1</a>')
  // Remove Twitch embeds
  content = content.replace(/<div class="player-contenu">\s*<div class="embed-responsive embed-responsive-16by9">\s*<iframe src="https:\/\/player\.twitch\.tv\/\?channel=([^&]+)&autoplay=false" allowfullscreen><\/iframe>\s*<\/div>\s*<\/div>/g, '<a class="link" href="https://www.twitch.tv/$1" target="_blank" title="$1">https://www.twitch.tv/$1</a>')

  // Cut long links
  content = content.replace(/<a(?: class="[^"]+")? ([^>]+)>([^<]{90,})<\/a>/g, (all, attributes, text) => {
    return `<a class="link long-link" ${attributes}>` + text.substr(0, 85) + '<span class="long-link__hidden-part">' + text.substr(85) + '</span></a>'
  })
